...
type SaleItem = {
  id:string;sales_transaction_id:string;gpos_item_name:string;menu_item_id:string|null;recipe_id:string|null;
  quantity:number;net_amount:number;unit_cost_snapshot:number|null;total_cost_snapshot:number|null;
  mapping_status:string;costing_status:CostingStatus;
};
...
const statusLabel:Record<string,string>={PROCESSED:'ตัดสต็อกแล้ว',PENDING:'รอตัดสต็อก',MISSING_COST:'ต้นทุนไม่ครบ',ERROR:'ต้องตรวจสอบ'};

export function SalesInventoryUsagePage(){
  const[searchParams]=useSearchParams();
  const invoiceParam=searchParams.get('invoice')??'';
  const[items,setItems]=useState<SaleItem[]>([]),[transactions,setTransactions]=useState<Transaction[]>([]),[menus,setMenus]=useState<MenuItem[]>([]),[movements,setMovements]=useState<Movement[]>([]),[ingredients,setIngredients]=useState<Ingredient[]>([]),[units,setUnits]=useState<Unit[]>([]);
  const[selected,setSelected]=useState(''),[query,setQuery]=useState(invoiceParam),[status,setStatus]=useState('ALL'),[menuId,setMenuId]=useState('ALL'),[dateFrom,setDateFrom]=useState(''),[dateTo,setDateTo]=useState(''),[role,setRole]=useState<Role>('STAFF'),[msg,setMsg]=useState('');
  ...
  const load=useCallback(async()=>{
    ...
    if(authData.user){const{data:profile}=await supabase.from('profiles').select('role').eq('id',authData.user.id).maybeSingle();if(profile?.role==='OWNER'||profile?.role==='MANAGER'||profile?.role==='STAFF')setRole(profile.role)}
    const[a,b,c,d,e,f]=await Promise.all([
      ...
      supabase.from('menu_items').select('id,name_th,name_en').order('name_th'),
  ...
  const filtered=useMemo(()=>items.filter(item=>{
    const tx=txMap.get(item.sales_transaction_id);const menu=item.menu_item_id?menuMap.get(item.menu_item_id):null;const name=menu?.name_th||menu?.name_en||item.gpos_item_name;const hay=`${tx?.invoice_no??''} ${name} ${item.gpos_item_name}`.toLowerCase();const soldDate=tx?.sold_at.slice(0,10)??'';
    if(query&&!hay.includes(query.toLowerCase()))return false;
    if(status!=='ALL'&&item.costing_status!==status)return false;
    if(menuId!=='ALL'&&item.menu_item_id!==menuId)return false;
    if(dateFrom&&soldDate<dateFrom)return false;
    if(dateTo&&soldDate>dateTo)return false;
    return true;
  }),[items,txMap,menuMap,query,status,menuId,dateFrom,dateTo]);

  const selectedItem=items.find(x=>x.id===selected)??null;
  const details:DetailRow[]=useMemo(()=>movements.filter(m=>m.sales_item_id===selected).map(m=>{const ing=ingredientMap.get(m.ingredient_id);const u=ing?unitMap.get(ing.base_unit_id):undefined;return{...m,ingredientName:ing?.name_th||ing?.name_en||m.ingredient_id,unitName:formatUnit(u)}}),[movements,selected,ingredientMap,unitMap]);

  const totals=useMemo(()=>{
    const processed=filtered.filter(x=>x.costing_status==='PROCESSED');
    const quantity=filtered.reduce((sum,x)=>sum+Number(x.quantity||0),0);
    const cost=processed.reduce((sum,x)=>sum+Number(x.total_cost_snapshot??0),0);
    const byMenu=new Map<string,{name:string;cost:number;quantity:number}>();
    for(const item of processed){const menu=item.menu_item_id?menuMap.get(item.menu_item_id):null;const name=menu?.name_th||menu?.name_en||item.gpos_item_name;const key=item.menu_item_id??`gpos:${item.gpos_item_name}`;const current=byMenu.get(key)??{name,cost:0,quantity:0};current.cost+=Number(item.total_cost_snapshot??0);current.quantity+=Number(item.quantity||0);byMenu.set(key,current)}
    const topMenu=[...byMenu.values()].sort((a,b)=>b.cost-a.cost)[0]??null;
    return{processed:processed.length,pending:filtered.filter(x=>x.costing_status==='PENDING').length,problem:filtered.filter(x=>x.costing_status==='MISSING_COST'||x.costing_status==='ERROR').length,cost,quantity,avgCost:quantity>0?cost/quantity:0,topMenu};
  },[filtered,menuMap]);

  async function processPending(){
    if(role!=='OWNER'&&role!=='MANAGER')return;setBusy(true);setMsg('');
    ...
    const{data,error}=await supabase.rpc('process_pending_sales_inventory',{p_sales_item_ids:ids});setBusy(false);
    if(error){setMsg(uiError('ประมวลผลการตัดสต็อก'));return}
    const result=data as {processed?:number;missing_cost?:number;errors?:number}|null;setMsg(`ประมวลผลแล้ว ${result?.processed??0} รายการ · ต้นทุนไม่ครบ ${result?.missing_cost??0} · ผิดพลาด ${result?.errors??0}`);await load();
  }

  function clearFilters(){setQuery('');setStatus('ALL');setMenuId('ALL');setDateFrom('');setDateTo('');}
  ...
  return <>
    ...
    <div className="cards compact">
      ...
      <div className="card"><small>ต้นทุนเฉลี่ยต่อหน่วยขาย</small><strong>฿{totals.avgCost.toFixed(2)}</strong></div>
    ...
    {totals.topMenu&&<div className="panel"><h2>สรุปต้นทุนตามตัวกรอง</h2><p>เมนูที่มีต้นทุนวัตถุดิบรวมสูงสุด: <strong>{totals.topMenu.name}</strong> · ขาย {totals.topMenu.quantity.toFixed(2)} · ต้นทุนรวม ฿{totals.topMenu.cost.toFixed(2)}</p></div>}

    <div className="panel"><div className="toolbar">
      <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} aria-label="วันที่เริ่มต้น" title="วันที่เริ่มต้น"/>
      <input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} aria-label="วันที่สิ้นสุด" title="วันที่สิ้นสุด"/>
      <select value={menuId} onChange={e=>setMenuId(e.target.value)} aria-label="เมนู"><option value="ALL">ทุกเมนู</option>{menus.map(m=><option key={m.id} value={m.id}>{m.name_th||m.name_en||m.id}</option>)}</select>
      <input placeholder="ค้นหาเมนูหรือเลขที่ใบขาย..." value={query} onChange={e=>setQuery(e.target.value)}/>
      <select value={status} onChange={e=>setStatus(e.target.value)}><option value="ALL">ทุกสถานะ</option><option value="PROCESSED">ตัดสต็อกแล้ว</option><option value="PENDING">รอตัดสต็อก</option><option value="MISSING_COST">ต้นทุนไม่ครบ</option><option value="ERROR">ต้องตรวจสอบ</option></select>
      <button className="secondary" onClick={clearFilters}>ล้างตัวกรอง</button><button className="secondary" onClick={()=>void load()}>รีเฟรช</button>
    </div>
      {invoiceParam&&<p className="muted">กำลังแสดงรายการจากใบขาย: <strong>{invoiceParam}</strong></p>}
      {msg&&<div className="warn">{msg}</div>}
      <div className="tablewrap"><table><thead><tr><th>วัน-เวลา</th><th>เลขที่ใบขาย</th><th>เมนู</th><th>จำนวนขาย</th><th>ยอดขาย</th><th>ต้นทุน/หน่วยขาย</th><th>ต้นทุนรวม</th><th>สถานะ</th><th>รายละเอียด</th></tr></thead><tbody>
