...
const sections = [
  ...
  ['เมนูสินค้า','/menu'],['สูตรเครื่องดื่ม','/recipes'],['ต้นทุนสูตร','/recipe-costing'],['ภาพรวมสต็อก','/inventory'],['ความเคลื่อนไหวสต็อก','/inventory/movements'],
...
function Shell(){
  ...
  return <div className="app">
    ...
    <main className="appMain"><Routes>
      ...
      {sections.filter(([,p])=>!['/','/sales','/sales/transactions','/sales/import','/sales/import-history','/recipes','/recipe-costing','/inventory','/inventory/movements','/inventory/count','/inventory/waste','/inventory/low','/purchasing','/expenses','/reports','/settings/users','/menu','/suppliers','/settings','/settings/units','/settings/categories','/ingredients'].includes(p)).map(([n,p])=><Route key={p} path={p} element={<ComingSoon title={n}/>}/>)}</Routes></main>
  </div>
}
