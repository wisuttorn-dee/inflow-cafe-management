...
const navGroups: NavGroup[] = [
  ...
  { label:'จัดซื้อและค่าใช้จ่าย', items:[
...
export function Sidebar({ open, onNavigate }: { open:boolean; onNavigate:()=>void }) {
  ...
  useEffect(()=>{void supabase.auth.getUser().then(async({data})=>{
    const user=data.user;if(!user)return;setEmail(user.email??'');
  ...
  const visibleGroups=useMemo(()=>navGroups.map(group=>({...group,items:group.items.filter(item=>!item.roles||item.roles.includes(role))})).filter(group=>group.items.length>0),[role]);

  return <aside className={open?'sidebar open':'sidebar'}>
    <div className="sidebarBrand"><div className="brandMark"><Coffee size={20} strokeWidth={2.2}/></div><div><strong>INFLOW</strong><span>Riverside Café</span></div></div>
    <nav className="sidebarNav" onClick={onNavigate}>{visibleGroups.map(group=><section className="navGroup" key={group.label}><div className="navGroupLabel">{group.label}</div>{group.items.map(item=>{const Icon=item.icon;return <NavLink key={item.path} to={item.path} end={item.path==='/'} className="navItem"><Icon size={17} strokeWidth={1.9}/><span>{item.label}</span><ChevronRight className="navChevron" size={14}/></NavLink>})}</section>)}</nav>
    <div className="sidebarFooter"><div className="userMiniCard"><div className="avatar">{displayName.slice(0,1).toUpperCase()}</div><div className="userMeta"><strong>{displayName}</strong><span>{email}</span></div><span className={`roleBadge role${role}`}>{roleLabel[role]}</span></div><button className="signoutButton" onClick={()=>void supabase.auth.signOut()}><LogOut size={16}/> ออกจากระบบ</button></div>
  </aside>;
}
